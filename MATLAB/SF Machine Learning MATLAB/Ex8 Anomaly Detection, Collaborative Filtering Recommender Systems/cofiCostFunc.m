...
J = sum(sum(diff.^2)) /2;
J = J + lambda/2 * sum(sum(X.^2));      % add regularised term of x
J = J + lambda/2 * sum(sum(Theta.^2));  % add regularised term of theta


% Compute gradients
% For X gradient
X_grad = diff * Theta;                   % unregularised vectorised implementation
X_grad = X_grad + (lambda * X);             % regularised

% For Theta gradient
Theta_grad = (diff' * X);                 % unregularised vectorised implementation
